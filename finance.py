...
────────────────────────────────
        ...
        if eps_valid and not valid_peer_pe.empty:
            ...
            industry_pe_avg = implied_price_avg = implied_price_min = 
...
"Prediction"]):
            ...
            for peer_idx in peer_indices:
                ...
                peer_eps = eps_data.loc[peer_idx].mask(eps_data.loc[peer_idx] 
<= 0)
                ...
                for yr in range(2010, 2024):
                    if (
                        ...
                        err = abs(
                            (peer_model[yr] - peer_actual[yr + 1]) / peer_actual[yr 
+ 1]
                        ) * 100
                        errors.append(err)
            if errors:
...
radius:8px;}
.metric-item {border:1px solid #ddd;border-
radius:6px;padding:10px;text-align:center;font-
